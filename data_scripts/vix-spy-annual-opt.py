...
def plot_best_annual_returns(spy_df, vix_df, start_date, end_dates):
    def compute_annual_return(end):
        ...
        return annual_return * 100, end[:4]

    results = Parallel(n_jobs=-1)(delayed(compute_annual_return)(end) for end in end_dates)
    annual_returns, years = zip(*results)
