...
def plot_cumulative_returns(spy_df, vix_df, strategy, start_date, end_date):
    spy_df = implement_strategy(spy_df.copy(), vix_df, strategy)
    spy_df.loc[:, 'Cumulative Return'] = (1 + spy_df['Portfolio Return']).cumprod()
    
    plt.figure(figsize=(14, 7))
    plt.plot(spy_df.loc[start_date:end_date].index, spy_df.loc[start_date:end_date, 'Cumulative Return'], label=f'Cumulative Return from {start_date} to {end_date}')
    plt.xlabel('Date')
    plt.ylabel('Cumulative Return (%)')
    plt.title(f'Cumulative Return from {start_date} to {end_date}')
    plt.legend()
    plt.grid(True)
    plt.show()
...
def main():
    ...
    for end in end_dates:
        ...
        plot_cumulative_returns(spy_df, vix_df, optimal_strategy, start_date, end)
